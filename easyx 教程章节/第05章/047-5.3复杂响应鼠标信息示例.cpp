...
int main()
{
	...
	// 绘制边框,画空心矩形，用于显示响应鼠标的特殊区域，实际使用时可删除本块内容
	...
	{	
		...
		// 2.监听鼠标信息;当有鼠标消息的时候执行,可检测连续的鼠标信息
		{
			...
			// 4.判断鼠标信息;鼠标左键按下
			{
				if (msg.x > 10 && msg.x < 100 && msg.y > 20 && msg.y < 120)
				// 4.判断鼠标信息;判断是否在特殊区域，该区域左上坐标(10,20)，右下坐标(100,120)
				// 此处还可以设置其他条件，只要逻辑正确即可。如点击一次后就不再响应等
				{
					closegraph();
					return 0;
					// 5.响应鼠标点击的对应的操作 退出程序
